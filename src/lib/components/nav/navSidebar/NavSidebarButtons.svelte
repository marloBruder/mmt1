<script lang="ts">
  ...
  let { activeTab, isCollapsed, tabInfo, onClick = (i) => {} }: { activeTab: number; isCollapsed: boolean; tabInfo: { title: string; component: Component; icon: Component }[]; onClick: (tabIndex: number) => void } = $props();
